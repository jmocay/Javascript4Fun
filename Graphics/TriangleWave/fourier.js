    ...
    draw() {
        ...
        for (let x = 0; x < xTranslate; x++) {
            ...
            for (let k = 0; k < this.harmonics; k++) {
                y += -Math.sin(
                    2 * Math.PI * (2*k + 1) * (this.freq * this.t / 180 + x / this.lambda)
                ) * Math.pow(-1, k) * Math.pow(2*k + 1, -2)
            }
            y = this.amp * 8 / Math.pow(Math.PI, 2) * y
            if (x > 1) {
...
class Armature {
    ...
    draw() {
        ...
        for (let k = 0; k < this.harmonics; k++) {
            ...
            let r = this.amp * 8 * Math.pow(-1, k) * Math.pow(2*k + 1, -2) / Math.pow(Math.PI, 2)
            let theta = 2 * Math.PI * (2*k + 1) * this.freq * this.t / 180
            ...
            ctx.lineWidth = 1
            ctx.beginPath()
            ctx.arc(x0 - xOffset, y0, Math.abs(r), 0, 2 * Math.PI)
            ctx.stroke()
